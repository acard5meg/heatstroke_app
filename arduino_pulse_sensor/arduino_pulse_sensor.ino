...
void setup() {
  ...
  */
  ...
  pulseSensor.analogInput(PULSE_INPUT); // set pin pulse sensor connected to, has to be an analog. Generally, A0
  pulseSensor.blinkOnPulse(PULSE_BLINK); // set pin that'll blink to your pulse, this is LED_BUILTIN which is 13
  pulseSensor.fadeOnPulse(PULSE_FADE); // sets pin to fade uses pulse with modulation

  pulseSensor.setSerial(Serial); // include so the playground outputs serial data automatically
  pulseSensor.setOutputType(OUTPUT_TYPE); // output in SERIAL_PLOTTER style
  pulseSensor.setThreshold(THRESHOLD); // sets value pulsesensor signal has to cross when going up, useful for combatting noise
